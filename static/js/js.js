...
js.deptiframe=function(tit,w,h,url){
	var mxw= w;
	var hm = h;
//	if(hm>800)hm=800;if(hm<400)hm=400;
	if(url.indexOf('wintype=max')>0){
		mxw= 900;
		hm=winHb()-45;
	}
//	var wi = winWb()-150;if(wi>mxw)wi=mxw;if(wi<700)wi=700;
	js.tanbody('winiframe',tit,w,h,{
		html:'<div id="openinputiframe" width="100%" height="100%"></div>',
		bbar:'none'
	});
	$.ajax({
		type:"post",
...
js.tanbody=function(act,title,w,h,can1){
	...
	this.tanoffset(act,t);
	can.showfun(act);
}
js.tanoffset=function(act,tt=0){
	var mid=''+act+'_main';
...
js.iframeclose=function(act,guan) {
	if(!isempt(guan)){
		var s= guan.split(',');
		for(var i=0;i<s.length;i++)$('#'+s[i]+'_main').remove();
	}
	js.xpbodyiframe(act,'none');
	$('#'+act+'_main',window.parent.document).remove();
	return false;
}
js.xpbodyiframe=function(act,type){
	if(type=='none'){
		$("div[xpbody='"+act+"']",window.parent.document).remove();
		if(!get('xpbg_bodydds'))$('div[tanbody]',window.parent.document).remove();
		return;
	}
	...
	$('body',window.parent.document).prepend(bs);	
	$('#xpbg_bodydds',window.parent.document).fadeIn(300);
}
...
js.changeproduct=function(na, lx, tits,ocans){
...
function click_manutwo(self){
	var u = self.getAttribute("url");
